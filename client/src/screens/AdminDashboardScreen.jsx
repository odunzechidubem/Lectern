    ...
    <div>
      <div className="flex flex-col mb-8 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="mt-4 sm:mt-0 sm:hidden">
          ...
          <select id="tabs" name="tabs" className="block w-full border-gray-300 rounded-md focus:border-indigo-500 focus:ring-indigo-500" value={activeTab} onChange={(e) => setActiveTab(e.target.value)}>
      ...
      <div>
            ...
            {isLoadingUsers ? <Loader /> : usersError ? <Message variant="error">{usersError?.data?.message || usersError.error}</Message> : (
              <ul className="divide-y divide-gray-200">
                {users && users.length > 0 ? (users.map(user => (
                  <li key={user._id} className="flex flex-col justify-between p-4 sm:flex-row sm:items-center">
                    <div className="flex items-center">
                      <img src={user.profileImage || `https://ui-avatars.com/api/?name=${user.name.split(' ').join('+')}&background=random`} alt="Profile" className="object-cover w-10 h-10 mr-4 rounded-full" />
                      <div>
                    ...
                    <div className="flex items-center mt-4 space-x-2 self-start sm:self-center sm:space-x-4 sm:mt-0">
                      ...
                      <button onClick={() => handleToggle(user._id)} disabled={isToggling} className="px-3 py-1 text-xs text-white bg-yellow-500 rounded hover:bg-yellow-600">{user.isActive ? 'Disable' : 'Enable'}</button>
                      <button onClick={() => handleDeleteUser(user._id)} disabled={isDeletingUser} className="p-2 text-xs text-white bg-red-600 rounded-full hover:bg-red-700"><FaTrash /></button>
                    </div>
                  </li>
            ...
            {isLoadingCourses ? <Loader /> : coursesError ? <Message variant="error">{coursesError?.data?.message || coursesError.error}</Message> : (
              <ul className="divide-y divide-gray-200">
                {courses && courses.length > 0 ? (courses.map(course => (
                  <li key={course._id} className="flex flex-col justify-between p-4 sm:flex-row sm:items-center">
                    <div>
                      <p className="font-semibold text-gray-800">{course.title}</p>
                      <p className="text-sm text-gray-600">Lecturer: {course.lecturer?.name || 'N/A'}</p>
                    </div>
                    <div className="mt-4 sm:mt-0">
                      <button onClick={() => handleDeleteCourse(course._id)} disabled={isDeletingCourse} className="p-2 text-xs text-white bg-red-600 rounded-full hover:bg-red-700"><FaTrash /></button>
                    </div>
                  </li>
        ...
        {activeTab === 'articles' && (
            <div className="space-y-6">
                <form onSubmit={handleCreateArticle} className="p-6 space-y-4 bg-white rounded-lg shadow-md">
                    <h2 className="text-2xl font-bold">Create New Article</h2>
                    <div><label htmlFor="articleTitle" className="block mb-1 font-bold text-gray-700">Title</label><input type="text" id="articleTitle" value={articleTitle} onChange={(e) => setArticleTitle(e.target.value)} className="w-full px-3 py-2 border rounded" required /></div>
                    <div><label htmlFor="articleDescription" className="block mb-1 font-bold text-gray-700">Description</label><textarea rows="3" id="articleDescription" value={articleDescription} onChange={(e) => setArticleDescription(e.target.value)} className="w-full px-3 py-2 border rounded" required /></div>
                    <div><label htmlFor="articleFile" className="block mb-1 font-bold text-gray-700">Article PDF File</label><input type="file" id="articleFile" onChange={(e) => setArticleFile(e.target.files[0])} className="w-full" accept=".pdf" /></div>
                                    ...
                                    {editingArticle?._id === article._id ? (
                                        <form onSubmit={handleUpdateArticle} className="space-y-2">
                                            ...
                                            <div className="flex gap-2">
                                                <button type="submit" disabled={isUpdatingArticle} className="px-3 py-1 text-sm text-white bg-green-500 rounded">Save</button>
                                                <button type="button" onClick={() => setEditingArticle(null)} className="px-3 py-1 text-sm bg-gray-200 rounded">Cancel</button>
                                            </div>
                                        </form>
                                    ) : (
                                        <div className="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500">{article.publicPages} pages visible</p>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <button onClick={() => handleEditArticleClick(article)} className="p-2 rounded-full hover:bg-gray-100"><FaEdit /></button>
                                                <button onClick={() => handleDeleteArticle(article._id)} disabled={isDeletingArticle} className="p-2 text-red-500 rounded-full hover:bg-red-100"><FaTrash /></button>
                                            </div>
                                        </div>
                                    )}
                                </li>
        ...
        {activeTab === 'siteContent' && (
            isLoadingSettings ? <Loader /> : (
                <form onSubmit={handleSubmitContent} className="p-6 space-y-6 bg-white rounded-lg shadow-md">
                    <h3 className="text-xl font-bold">General Settings</h3>
                    <div><label className="block mb-2 font-bold text-gray-700">Site Name</label><input type="text" name="siteName" value={formState.siteName || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">Logo</label><img src={formState.logoUrl || '/logo.png'} alt="Logo Preview" className="h-12 p-1 mb-2 bg-gray-200 rounded w-auto" /><input type="file" name="logoUrl" data-label="Logo" onChange={handleFileUpload} className="w-full" accept="image/*" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">Favicon</label><img src={formState.faviconUrl || '/vite.svg'} alt="Favicon Preview" className="w-8 h-8 p-1 mb-2 bg-gray-200 rounded" /><input type="file" name="faviconUrl" data-label="Favicon" onChange={handleFileUpload} className="w-full" accept="image/png, image/svg+xml, image/x-icon" /></div>
                    <hr /><h3 className="text-xl font-bold">Home Page (Hero Section)</h3>
                    <div><label className="block mb-2 font-bold text-gray-700">Hero Title</label><input type="text" name="heroTitle" value={formState.heroTitle || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">Hero Text</label><textarea name="heroText" rows="3" value={formState.heroText || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">Hero Image</label><img src={formState.heroImageUrl || ''} alt="Hero Preview" className="object-cover w-auto h-32 mb-2 rounded" /><input type="file" name="heroImageUrl" data-label="Hero Image" onChange={handleFileUpload} className="w-full" accept="image/*" /></div>
                    <hr /><h3 className="text-xl font-bold">About Us Page</h3>
                    <div><label className="block mb-2 font-bold text-gray-700">About Us Title</label><input type="text" name="aboutUsTitle" value={formState.aboutUsTitle || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">About Us Text</label><textarea name="aboutUsText" rows="5" value={formState.aboutUsText || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">About Us Image</label><img src={formState.aboutUsImageUrl || ''} alt="About Us Preview" className="object-cover w-auto h-32 mb-2 rounded" /><input type="file" name="aboutUsImageUrl" data-label="About Us Image" onChange={handleFileUpload} className="w-full" accept="image/*" /></div>
                    <hr /><h3 className="text-xl font-bold">Footer Settings</h3>
                    <div><label className="block mb-2 font-bold text-gray-700">Footer About Text</label><input type="text" name="footerAboutText" value={formState.footerAboutText || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">Contact Email</label><input type="email" name="footerContactEmail" value={formState.footerContactEmail || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <div><label className="block mb-2 font-bold text-gray-700">Contact Phone</label><input type="text" name="footerContactPhone" value={formState.footerContactPhone || ''} onChange={handleInputChange} className="w-full px-3 py-2 border rounded" /></div>
                    <button type="submit" disabled={isUpdatingSettings || isUploading} className="w-full py-2 text-white bg-blue-500 rounded hover:bg-blue-600">{isUpdatingSettings || isUploading ? 'Saving...' : 'Save All Text Changes'}</button>
                </form>
            )
        )}
            ...
            <div className="p-6 space-y-6 bg-white rounded-lg shadow-md">
                <form onSubmit={handleCreateLink} className="pb-6 mb-6 border-b">
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row">
                        <input type="text" placeholder="Link Title" value={newLinkTitle} onChange={(e) => setNewLinkTitle(e.target.value)} className="w-full px-3 py-2 border rounded" />
                        <input type="text" placeholder="URL" value={newLinkUrl} onChange={(e) => setNewLinkUrl(e.target.value)} className="w-full px-3 py-2 border rounded" />
                        <button type="submit" disabled={isCreatingLink} className="px-4 py-2 text-white bg-green-500 rounded whitespace-nowrap">{isCreatingLink ? 'Adding...' : 'Add Link'}</button>
                    </div>
                </form>
                        ...
                        {links && links.length > 0 ? links.map(link => (
                            <li key={link._id} className="py-2">
                                {editingLink?._id === link._id ? (
                                    <form onSubmit={handleUpdateLink} className="flex flex-col items-center gap-2 sm:flex-row">
                                        ...
                                        <button type="submit" disabled={isUpdatingLink} className="px-3 py-1 text-white bg-blue-500 rounded whitespace-nowrap">{isUpdatingLink ? '...' : 'Save'}</button>
                                        <button type="button" onClick={() => setEditingLink(null)} className="px-3 py-1 bg-gray-200 rounded whitespace-nowrap">Cancel</button>
                                    </form>
                                ) : (
                                    <div className="flex items-center justify-between">
                                        <div>
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <button onClick={() => handleEditClick(link)} className="p-2 rounded-full hover:bg-gray-100"><FaEdit /></button>
                                            <button onClick={() => handleDeleteLink(link._id)} disabled={isDeletingLink} className="p-2 text-red-500 rounded-full hover:bg-red-100"><FaTrash /></button>
                                        </div>
                                    </div>
                                )}
                            </li>

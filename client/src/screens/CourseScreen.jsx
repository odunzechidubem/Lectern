...
import { FaPlayCircle, FaFilePdf, FaClipboardList, FaBullhorn } from 'react-icons/fa';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              {userInfo && userInfo.role === 'student' && !isEnrolled && (
                <button onClick={handleEnroll} disabled={isEnrolling} className="mt-4 w-full bg-green-500 text-white font-semibold py-2 px-4 rounded hover:bg-green-600">{isEnrolling ? 'Enrolling...' : 'Enroll Now'}</button>
              )}
              {userInfo && userInfo.role === 'student' && isEnrolled && (
                  ...
                  <ul className="space-y-4">
                    {course.lectures.map((lecture, index) => (
                      <li key={lecture._id} className="bg-white flex justify-between items-center transition-shadow">
                        <Link to={`/course/${course._id}/lecture/${index}`} className="flex items-center group p-2 rounded-l-lg hover:bg-gray-100 flex-grow">
          ...
          <div className="space-y-6">
            {(isEnrolled || userInfo?.role === 'lecturer') && (
              <>
                <div className="bg-white p-6 rounded-lg shadow-md">
                  <h2 className="text-2xl font-semibold mb-4 flex items-center"><FaBullhorn className="mr-3 text-indigo-500" />Announcements</h2>
                  {isLoadingAnnouncements ? <Loader /> : announcements && announcements.length > 0 ? (
                    <ul className="space-y-4">
                      {announcements.map(ann => (
                        <li key={ann._id} className="border-l-4 border-indigo-500 pl-4">
                    ...
                    <ul className="space-y-3">
                      {course.assignments.map(assignment => (
                        <li key={assignment._id}>
                          <Link to={`/course/${courseId}/assignment/${assignment._id}`} className="flex items-center p-3 rounded-lg hover:bg-gray-100 w-full text-left">
            ...
            {!isEnrolled && userInfo?.role === 'student' && (
              <div className="bg-white p-6 rounded-lg shadow-md">
                <Message>Enroll in this course to view announcements, lectures, and assignments.</Message>
              </div>
            )}
          </div>
        </div>

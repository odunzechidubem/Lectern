...
const verifyEmailChange = asyncHandler(async (req, res) => { const hashedToken = crypto.createHash('sha256').update(req.params.token).digest('hex'); const user = await User.findOne({ emailChangeToken: hashedToken, emailChangeTokenExpires: { $gt: Date.now() }, }); if (!user) { res.status(400); throw new Error('Token is invalid or has expired.'); } user.email = user.newEmail; user.newEmail = undefined; user.emailChangeToken = undefined; user.emailChangeTokenExpires = undefined; await user.save(); const { userSocketMap, io } = req; const userSocketId = userSocketMap.get(user._id.toString()); if (userSocketId) { io.to(userSocketId).emit('force-logout', { message: 'Your email has been changed. Please log in with your new email address.', }); } res.status(200).send('<h1>Email Successfully Updated</h1><p>Your email address has been changed. Your previous session has been logged out. You can now close this tab and log in with your new email.</p>'); });

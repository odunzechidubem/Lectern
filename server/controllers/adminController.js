...
const getAllUsers = asyncHandler(async (req, res) => {
  ...
  if (role !== 'student' && role !== 'lecturer') { res.status(400); throw new Error("Invalid role specified. Must be 'student' or 'lecturer'."); }
...
const toggleUserStatus = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    if (user._id.toString() === req.user._id.toString()) { res.status(400); throw new Error('Administrators cannot change their own status.'); }
    ...
    if (!user.isActive) {
      ...
      if (userSocketId) {
        io.to(userSocketId).emit('force-logout', { message: 'Your account has been disabled by an administrator.' });
...
const deleteUserById = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    if (user._id.toString() === req.user._id.toString()) { res.status(400); throw new Error('Administrators cannot delete their own account.'); }
...
const deleteCourseById = asyncHandler(async (req, res) => {
  ...
  if (course) {
    ...
    if (publicIdsToDelete.length > 0) {
      console.log(`[Admin] Deleting ${publicIdsToDelete.length} assets from Cloudinary for course ${course.title}.`);
      try {
        await Promise.all(publicIdsToDelete.map(asset => 
          cloudinary.uploader.destroy(asset.id, { resource_type: asset.type })
        ));
      } catch (err) {
        console.error("Error during bulk deletion of course assets by admin. Continuing DB cleanup.", err);
      }
    }
